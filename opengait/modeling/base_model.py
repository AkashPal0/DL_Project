...
__all__ = ['BaseModel'] ## __all__ is used to export all the names in a module when from <module> import * is used
...
class BaseModel(MetaModel, nn.Module):
    ...
    def __init__(self, cfgs, training):
        ...
        self.build_network(cfgs['model_cfg']) # Network build using GaitGL class
        self.init_parameters()
        self.trainer_trfs = get_transform(cfgs['trainer_cfg']['transform']) ## transform:  <class 'data.transform.BaseSilCuttingTransform'> object // used in inputs_pretreament function
        ...
        self.train(training) # Defined in module.py
    ...
    def init_parameters(self):
        for m in self.modules():
            if isinstance(m, (nn.Conv3d, nn.Conv2d, nn.Conv1d)): #The isinstance() function returns True if the specified object is of the specified type, otherwise False.
    ...
    def get_loader(self, data_cfg, train=True):
        ...
        Sampler = get_attr_from([Samplers], sampler_cfg['type']) ##<class 'data.sampler.TripletSampler'>
        vaild_args = get_valid_args(Sampler, sampler_cfg, free_keys=[
            'sample_type', 'type']) ## {'batch_shuffle': True, 'batch_size': [8, 8]}
    ...
    def train_step(self, loss_sum) -> bool:
        """Conduct loss_sum.backward(), self.optimizmodel.train_loaderer.step() and self.scheduler.step().
    ...
    def inference(self, rank): ###rank is zero, Rank is the unique ID givenModel to a process, so that other processes know how to identify a particular process. Local rank is the a unique local ID for processes running in a single node
        ...
        """
        total_size = len(self.test_loader) ## total sequences in test_loader are 5485
        if rank == 0:
        ...
        info_dict = Odict()
        for inputs in self.test_loader: ## inputs = [s,h,w] === [77,64,64]
            ipts = self.inputs_pretreament(inputs) #ipts = [c,s,h,w] === [1,77,64,44]
            with autocast(enabled=self.engine_cfg['enable_float16']):
                retval = self.forward(ipts)
                inference_feat = retval['inference_feat'] ###[1,128,64]
    ...
    def run_train(model):
        ...
        for inputs in model.train_loader: # Object of get_loader, it also call TripleSampler __iter__ method
            ipts = model.inputs_pretreament(inputs)
            with autocast(enabled=model.engine_cfg['enable_float16']):
                retval = model(ipts) ## GaitGL forward() method called
