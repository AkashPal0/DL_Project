...
class GLConv(nn.Module):
    ...
    def forward(self, x):
        ...
        else: ## this runs in GaitGL
            h = x.size(3) ## height of frame [64] x ==> [64,32,10,64,44]
            split_size = int(h // 2**self.halving)
            lcl_feat = x.split(split_size, 3) #3 is place of splitting lcl_feat==> [64,32,10,8,44], 8 times
...
class GaitGL(BaseModel):
    ...
    def __init__(self, *args, **kargs):
        super(GaitGL, self).__init__(*args, **kargs) ## its parent BaseModel constructor called
    ...
    def forward(self, inputs):
        ...
        del ipts
        n, _, s, h, w = sils.size() ##train (64 , 1, 30, 64, 44) ###test [1,1,77,64,44]
        if s < 3:
            repeat = 3 if s == 1 else 2
            sils = sils.repeat(1, 1, repeat, 1, 1)

        outs = self.conv3d(sils) ## (64, 32, 30, 64, 44) ###test [1,32,77,64,44]
        outs = self.LTA(outs) # (64, 32, 10, 64, 44) ###test [1,32,25,64,44]
        ...
        if self.Bn_head:  # Original GaitGL Head
            ...
            logi = self.Head1(bnft)  # [n, c, p] #[64,74,64]
            embed = bnft
