...
class TripletSampler(tordata.sampler.Sampler):
    def __init__(self, dataset, batch_size, batch_shuffle=False):
        ...
        self.batch_size = batch_size # [8,8] for GaitGL
...
def sync_random_sample_list(obj_list, k, common_choice=False): ## k is batch_size[0] = 8 / batch_size[1] = 8
    if common_choice:
    ...
    if len(obj_list) < k:
        ...
        idx = torch.randperm(len(obj_list))[:k] ##torch.randperm(4) => tensor([2, 1, 0, 3])
