...
class BaseSilCuttingTransform():
    ...
    def __call__(self, x):
        # # print("x shape: ", x.shape) ### [s, h, w] ===== [30, 64, 64] ====== [seq_len, h, w]
        # if self.cutting is not None:
        #     cutting = self.cutting
        # else:
        #     cutting = int(x.shape[-1] // 64) * 10
...
def get_transform(trf_cfg=None):
    if is_dict(trf_cfg):
        # getattr is used to instantiate the BaseSilCuttingTransform object through base_transform module
        transform = getattr(base_transform, trf_cfg['type']) ## trf_cfg['type'] ====== BaseSilCuttingTransform //from default.yaml
