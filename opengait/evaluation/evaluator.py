...
def single_view_gallery_evaluation(feature, label, seq_type, view, dataset, metric):
    ...
    msg_mgr = get_msg_mgr()
    acc = {}
    view_list = sorted(np.unique(view)) # ['000', '018', '036', '054', '072', '090', '108', '126', '144', '162', '180']
    num_rank = 1
    if dataset == 'CASIA-E':
    ...
    for (type_, probe_seq) in probe_seq_dict[dataset].items():
        acc[type_] = np.zeros((view_num, view_num, num_rank)) - 1. # (11, 11, 1)
        for (v1, probe_view) in enumerate(view_list):
            ...
            probe_x = feature[pseq_mask, :] ## only 100 or few times true
            probe_y = label[pseq_mask]

            for (v2, gallery_view) in enumerate(view_list):
                gseq_mask = np.isin(seq_type, gallery_seq_dict[dataset]) & np.isin(view, [gallery_view])
                ...
                reshape_ = np.reshape(probe_y, [-1, 1]) == gallery_y[idx[:, 0:num_rank]]
                cumsum_ = np.cumsum(reshape_, 1)
                sum_ = np.sum(cumsum_ > 0, 0)
                acc[type_][v1, v2, :] = np.round(sum_ * 100 / dist.shape[0], 2)
...
def evaluate_indoor_dataset(data, dataset, metric='euc', cross_view_gallery=False):
    feature, label, seq_type, view = data['embeddings'], data['labels'], data['types'], data['views'] ## feature size [5485,128,64]
    label = np.array(label) #size = 5485
    view = np.array(view) #size = 5485

    if dataset not in ('CASIA-B', 'OUMVLP', 'CASIA-E', 'SUSTech1K'):
        raise KeyError("DataSet %s hasn't been supported !" % dataset)
    if cross_view_gallery: ## False for GaitGL
        return cross_view_gallery_evaluation(
            feature, label, seq_type, view, dataset, metric)
    else:
        return single_view_gallery_evaluation(
            feature, label, seq_type, view, dataset, metric) # metric = 'euc'
